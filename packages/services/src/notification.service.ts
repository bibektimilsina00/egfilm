...
export async function sendWatchInvite(
    ...
    embedUrl: string,
    extras?: { sport?: string | null; matchExternalId?: string | null; notificationType?: NotificationType }
) {
    try {
        ...
        const notification = await createNotification({
            type: extras?.notificationType ?? (mediaType === 'match' ? 'match_invite' : 'watch_invite'),
            title: mediaType === 'match' ? 'Match Watch-Together Invitation' : 'Watch Together Invitation',
            ...
            matchExternalId: extras?.matchExternalId ?? null,
        });
